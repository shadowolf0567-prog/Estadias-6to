...
$sql="INSERT INTO clientes (nombre,no_cuenta,encargado,direccion) VALUES (?,?,?,?)";
$stmt=mysqli_prepare($conn,$sql);
mysqli_stmt_bind_param($stmt, 'ssss', $nombre,$no_cuenta,$encargado,$direccion);
...
if(isset($data['telefonos']) && is_array($data['telefonos']) && count($data['telefonos']) > 0){
    ...
    foreach($data['telefonos'] as $telefono){
        if(!empty($telefono['numero'])){
            ...
            mysqli_stmt_bind_param($stmt_telefono,'iss',$id_cliente,$telefono['numero'],$contacto);
...
if(isset($data['correos']) && is_array($data['correos']) && count($data['correos']) > 0){
    ...
    foreach($data['correos'] as $correo){
        if(!empty($correo['direccion'])){
            ...
            mysqli_stmt_bind_param($stmt_correo,'iss',$id_cliente,$correo['direccion'],$contacto);
